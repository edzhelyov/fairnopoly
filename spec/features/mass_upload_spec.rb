...
describe "Mass-upload" do
  ...
  context "for signed-in legal entity users" do
    ...
    it "should have a csv upload link" do
      should have_link(
        I18n.t('mass_upload.labels.upload_article_via_csv'),
        href: new_mass_upload_path
      )
    end
    ...
    describe "when uploading" do
      ...
      context "as a user with missing payment data -" do
        before do
          attach_file(
            'mass_upload_file', 'spec/fixtures/mass_upload_correct.csv'
          )
        ...
        describe "any payment data" do
          it "should show the correct error message" do
            page.html.should include(
              I18n.t(
                'mass_upload.errors.missing_payment_details',
                missing_payment: I18n.t(
                  'formtastic.labels.user.paypal_and_bank_account'
                ),
                link: edit_user_registration_path(legal_entity_user) +
          ...
          it "should show the correct error message" do
            page.html.should include(
              I18n.t(
                'mass_upload.errors.missing_payment_details',
                missing_payment: I18n.t(
                  'formtastic.labels.user.paypal_account'),
                link: edit_user_registration_path(legal_entity_user) +
                '#payment_information_step'
              )
            )
        ...
        context "and a valid csv file" do
          ...
          describe "activate articles" do
            ...
            it "should disable a activate article button after it is clicked" do
              # xpath is used because of identical buttons
              page.find(:xpath,
                        "/html/body/div[4]/div/ul/a[1]/form/div/input[2]").click
              should have_selector('input.Btn.Btn--green.Btn--greenSmall')
